...
public class PayServiceImpl implements IPayService{
    ...
    // 测试当面付2.0生成支付二维码
    public void trade_precreate(OrderVo orderVo) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        ...
        for (OrderItemVo orderItemVo:orderItemVoList){
            // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
            GoodsDetail goods1 = GoodsDetail.newInstance(String.valueOf(orderItemVo.getProductId()), orderItemVo.getProductName(), orderItemVo.getCurrentUnitPrice().longValue(), orderItemVo.getQuantity());
            // 创建好一个商品后添加至商品明细列表
            goodsDetailList.add(goods1);
        }
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("E:/Ideacode/zhifubao/qr-%s.png",
    ...
    public ServerResponse pay(Long orderNo) {
        ...
        //2.根据orderNo查询订单
        ServerResponse<OrderVo> serverResponse = iOrderService.findOrderByOrderNo(orderNo);
